    ...
    constructor(){
        ...
        this.state = {
          ...
          circles : [
            ...
            {degrees : 360,color1:'',color2:'',color3:'',color4:'',match : 0,duration : 1000,rotateInterpolate : '',animatedStyle : ''},
      ...
      randomStage = () => {
        // this.props.navigation.navigate('Level_Stage')
      ...
      modal(data,data2) {
        ...
        if(this.state.total_move <=3) { global.level_score[global.level_select - 1].star = 3; this.setState({star : 3})}
        else if(this.state.total_move <=7 && this.state.total_move >=4){ 
      ...
      checker = () => {
        ...
        var match8 = null;
        // First
        if(this.state.sample[2].color == this.state.sample2[0].color){
          ...
          if(global.Game_Audio_Clip == 'Enabled' && total[0].match == 0) SoundPlayer.playSoundFile('sparkle', 'wav');SoundPlayer.setVolume(1)
          total[0].match = 1;
          match = this.state.sample;match[this.state.sample[2].position].detection = true;match[this.state.sample[0].position].detection = false;
          match2 = this.state.sample2;match2[this.state.sample2[0].position].detection = true;match2[this.state.sample2[2].position].detection = false;
        }
        else{
          var total = this.state.circles; total[0].match = 0;
          match = this.state.sample;match[this.state.sample[2].position].detection = false;match[this.state.sample[0].position].detection = false;
          match2 = this.state.sample2;match2[this.state.sample2[0].position].detection = false;match2[this.state.sample2[2].position].detection = false;
        }

        // Seconds
        if(this.state.sample3[0].color == this.state.sample2[2].color){
          if(global.Game_Audio_Clip == 'Enabled' && total[1].match == 0) SoundPlayer.playSoundFile('sparkle', 'wav');SoundPlayer.setVolume(1)
            var total = this.state.circles; total[1].match = 1; 
            match3 = this.state.sample3;match3[this.state.sample3[0].position].detection = true;match3[this.state.sample3[2].position].detection = false;
            match4 = this.state.sample2;match4[this.state.sample2[2].position].detection = true;
        }
        else{
            var total = this.state.circles; total[1].match = 0; this.setState({GameStart:"Start"});
            match3 = this.state.sample3;match3[this.state.sample3[0].position].detection = false;match3[this.state.sample3[2].position].detection = false;
        }
        ...
        if(score == 2) {this.setState({disable:true},this.modal('sweet',require('./img/popout/sweet.png')));}
      ...
      const  Upper_Tab  = this.state.Upper_Tab;
        ...
        <View style={{flexDirection:'row',width:'100%',alignItems:'center',justifyContent:'center'}}>
            ...
            <TouchableOpacity style={styles.HeaderOpacity} disabled={true}>
                <Image source={require('./img/Top_Long_Blue.png')} style={{width: Upper_Tab.Blue,height: hp('8%'),resizeMode: 'stretch'}} />
            </TouchableOpacity>
            ...
            {/* <Text style={{color:'white',position:'relative',top:hp('1%'),fontSize:wp('7%'),fontWeight:'bold'}}>{this.state.scoring}/2({this.state.total_move})</Text> */}
            {/* <Text style={{color:'white',position:'relative',top:hp('1%'),fontSize:wp('7%'),fontWeight:'bold'}}>{this.state.scoring}/2</Text> */}
            <Text style={{color:'white',position:'relative',top:hp('1%'),fontSize:wp('6%'),fontWeight:'bold'}}>Level {global.level_select}</Text>
        </View>

        <View style={{width:wp(this.state.screen),paddingTop:screen.padding_top,height:hp('80%'),flexDirection:'column',marginLeft : 'auto',marginRight : 'auto',alignItems:'center'}}>
...
const styles = StyleSheet.create({
  ...
  // <TapGestureHandler onHandlerStateChange={(evt) => {this.setState({direction_axis : evt.nativeEvent})}}>
  //           <GestureRecognizer onSwipe={(direction, state) => this.onSwipe(direction, state,'0')} config={this.state.config}>
  //             <Animated.View style={[styles.circle,this.state.circles[0].rotateInterpolate,{width : wp("38%"),height : wp("38%")}]}>
  //               <View style={{ width: wp("33%"),height: wp("33%"),borderRadius: 1000,backgroundColor: '#666666',alignItems:'center',justifyContent:'center' }}>
  //                   <View style={[this.state.sample[0].detection ? styles.sample : styles.sample2,{ width: wp("14%"),height: wp("14%"),backgroundColor: this.state.circles[0].color1,position:'absolute',bottom:wp("22%"),borderWidth:2,borderColor:'white',transform:[{rotate:'0deg'}]}]}><View style={{ width: 7,height: 10,borderRadius: 15,backgroundColor: 'white',alignSelf:'flex-end',position:'absolute',top:this.state.width / 12,right:5,transform: [{ rotate: "45deg" }]}}></View></View>
  //                   <View style={{ width: this.state.width / 15,height: this.state.width / 15,borderRadius: 100,backgroundColor: '#480582',position:'absolute',borderWidth:2,borderColor:'white'}}></View>
  //                   <View style={[this.state.sample[1].detection ? styles.sample : styles.sample2,{ width: wp("14%"),height: wp("14%"),backgroundColor: this.state.circles[0].color2,position:'absolute',left:wp("22%"),borderWidth:2,borderColor:'white',transform:[{rotate:'90deg'}]}]}><View style={{ width: 7,height: 10,borderRadius: 15,backgroundColor: 'white',alignSelf:'flex-end',position:'absolute',top:this.state.width / 12,right:5,transform: [{ rotate: "45deg" }]}}></View></View>
  //                   <View style={[this.state.sample[2].detection ? styles.sample : styles.sample2,{ width: wp("14%"),height: wp("14%"),backgroundColor: this.state.circles[0].color3,position:'absolute',top:wp("22%"),borderWidth:2,borderColor:'white',transform:[{rotate:'180deg'}]}]}><View style={{ width: 7,height: 10,borderRadius: 15,backgroundColor: 'white',alignSelf:'flex-end',position:'absolute',top:this.state.width / 12,right:5,transform: [{ rotate: "45deg" }]}}></View></View>
  //                   <View style={[this.state.sample[3].detection ? styles.sample : styles.sample2,{ width: wp("14%"),height: wp("14%"),backgroundColor: this.state.circles[0].color4,position:'absolute',right:wp("22%"),borderWidth:2,borderColor:'white',transform:[{rotate:'270deg'}]}]}><View style={{ width: 7,height: 10,borderRadius: 15,backgroundColor: 'white',alignSelf:'flex-end',position:'absolute',top:this.state.width / 12,right:5,transform: [{ rotate: "45deg" }]}}></View></View>
  //               </View>
  //             </Animated.View>
  //           </GestureRecognizer>
  //         </TapGestureHandler>